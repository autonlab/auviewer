...
class diagnoseEEG:
    ...
    def bimodal_aEEG_SUPPRESSED_WITH_ICTAL(self):
        if (min(self.weights[1]) > 0.05) and (np.min(self.means[1]) < self.thresholds['EEG__LOW']) and (np.max(self.means[1]) > self.thresholds['EEG__HIGH_5']):
    ...
    def bimodal_aEEG(self):
        if min(self.weights[1]) > 0.05: 
            if np.max(self.means[1]) < self.thresholds['EEG__LOW']:
                return self.SUPPRESSED
            elif (np.min(self.means[1]) < self.thresholds['EEG__LOW']) and (np.max(self.means[1]) > self.thresholds['EEG__HIGH_5']):
                return self.SUPPRESSED_WITH_ICTAL
            elif (np.min(self.means[1]) < self.thresholds['EEG__LOW']) and (np.max(self.means[1]) < self.thresholds['EEG__HIGH_5']):
                return self.BURST_SUPRESSION
    ...
    def low_baseline_SUPPRESSED(self):
        if (not self.NoBaseline) and (self.low_baseline) and (not self.many_high_amp_spikes) and (not self.many_low_amp_spikes):
            return self.SUPPRESSED
        else:
            return self.ABSTAIN
    ...
    def high_baseline_infrequent_drops_NORMAL(self):
        if (not self.NoBaseline) and (not self.low_baseline) and (self.dur_low_amplitude_EEG <= self.thresholds['near_zero_duration_tol']):
            return self.NORMAL
        else:
            return self.ABSTAIN

    def high_baseline_frequent_drops_SUPPRESSED_WITH_ICTAL(self):
        if (not self.NoBaseline) and (not self.low_baseline) and (self.dur_low_amplitude_EEG > self.thresholds['near_zero_duration_tol']) and (self.bimodal_aEEG_BURST_SUPRESSION() != self.BURST_SUPRESSION):
            return self.SUPPRESSED_WITH_ICTAL
        else:
            return self.ABSTAIN
    ...
    def high_baseline_frequent_drops(self):
        ...
        if not self.low_baseline:
            if self.dur_low_amplitude_EEG <= self.thresholds['near_zero_duration_tol']:
                return self.NORMAL
            elif self.dur_low_amplitude_EEG > self.thresholds['near_zero_duration_tol'] and self.bimodal_aEEG_BURST_SUPRESSION() == self.BURST_SUPRESSION:
                return self.BURST_SUPRESSION
            else:
                return self.SUPPRESSED_WITH_ICTAL
        else: 
    ...
    def get_vote_vector(self):
        ...
        self.unimodal_aEEG_SUPPRESSED(), self.bimodal_aEEG_SUPPRESSED(), self.bimodal_aEEG_SUPPRESSED_WITH_ICTAL(),
        self.bimodal_aEEG_BURST_SUPRESSION(), self.bimodal_aEEG_NORMAL(), self.low_baseline_SUPPRESSED_WITH_ICTAL(),
        self.low_baseline_BURST_SUPRESSION(), self.low_baseline_SUPPRESSED(), self.high_baseline_infrequent_drops_NORMAL(), 
        self.high_baseline_frequent_drops_SUPPRESSED_WITH_ICTAL(), self.high_baseline_frequent_drops_BURST_SUPRESSION()]
    ...
    def number_to_label_map():
        return {
            ...
            0: 'NORMAL',
            1: 'SUPPRESSED',
            2: 'SUPPRESSED_WITH_ICTAL',
            3: 'BURST_SUPRESSION'
        }

    @staticmethod
    def get_LF_names():
        return ['bimodal_aEEG', 'low_baseline_aEEG', 'high_baseline_frequent_drops', 'very_spiky_aEEG_SUPPRESSED_WITH_ICTAL', 
        ...
        'unimodal_aEEG_NORMAL', 'unimodal_aEEG_SUPPRESSED', 'bimodal_aEEG_SUPPRESSED', 'bimodal_aEEG_SUPPRESSED_WITH_ICTAL',
    ...
    def getThresholdsForLabelers():
        return { "bimodal_aEEG": ["EEG__LOW", "EEG__HIGH_5"],
            "low_baseline_aEEG": ["n_high_amplitude_peaks_per_hour"],
            "high_baseline_frequent_drops": ["near_zero_duration_tol"],
            "very_spiky_aEEG_SUPPRESSED_WITH_ICTAL": ["n_high_amplitude_peaks_per_hour"],
            "well_separated_aEEG_modes_SUPPRESSED_WITH_ICTAL": ["min_separation"],
            "aEEG_NOT_near_zero_NORMAL": ["near_zero", "near_zero_duration_tol"],
            "high_aEEG_baseline_NORMAL_4": ["EEG__HIGH_4"],
            "high_aEEG_baseline_NORMAL_10": ["EEG__HIGH_10"],
            "unimodal_aEEG_NORMAL": ["EEG__LOW"],
            "unimodal_aEEG_SUPPRESSED": ["EEG__LOW"],
            "bimodal_aEEG_SUPPRESSED": ["EEG__LOW"],
            "bimodal_aEEG_SUPPRESSED_WITH_ICTAL": ["EEG__LOW", "EEG__HIGH_5"],
            "bimodal_aEEG_BURST_SUPRESSION": ["EEG__LOW", "EEG__HIGH_5"],
            "bimodal_aEEG_NORMAL": ["EEG__LOW"],
            "low_baseline_SUPPRESSED_WITH_ICTAL": ["n_high_amplitude_peaks_per_hour"],
            "low_baseline_BURST_SUPRESSION": ["n_high_amplitude_peaks_per_hour"],
            "low_baseline_SUPPRESSED": ["n_high_amplitude_peaks_per_hour"],
            "high_baseline_infrequent_drops_NORMAL": ["near_zero_duration_tol"],
            "high_baseline_frequent_drops_SUPPRESSED_WITH_ICTAL": ["near_zero_duration_tol"],
            "high_baseline_frequent_drops_BURST_SUPRESSION": ["near_zero_duration_tol"]
        }
