...
function convertToDateObjsAndUpdateExtremes(data) {
	...
	for(var i = 0; i < data.length; i++) {
		...
		// Convert the ISO8601-format string into a Date object.
		data[i][0] = new Date(data[i][0]).valueOf();
...
function instantiateDygraph(series, dataset) {
	...
	*/
	...
	p = Bokeh.Plotting.figure({
		...
		x_range: new Bokeh.Range1d({start: globalXExtremes[0], end: globalXExtremes[1]}),
	...
	p.line({field:"x"},{field:"y"}, {source: source});
...
function updateCurrentViewData(graph) {
	...
	globalAppConfig.verbose && console.log("Requesting data for view.");
	...
	x.onreadystatechange = function() {
		...
		if (this.readyState == 4 && this.status == 200) {
			...
			globalAppConfig.verbose && console.log("Applying data to view.");
